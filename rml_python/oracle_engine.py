# rml_python.oracle_engine � Production Oracle Engine
# Secure _q, connect/disconnect, try...finally cursor management
# This is the refined OracleEngine for RMLReportEngine, with fixes:
# - get_sequence_next_value uses _q(sequence) not bind
# - bulk_create uses try/finally cur.close()
# - All reflection/CRUD uses try/finally to prevent cursor leaks
# - Clean indentation (4 spaces, no hidden chars)
"""
...
def _q(ident: str) -> str:
    # quote identifier; only split schema.table on dots when all parts
    # are plain DB identifiers — Arabic aliases like "الراتب (ر.س)"
    # contain a literal dot and must be quoted whole.
    import re as _re_q
    s = str(ident)
    if _re_q.match(r'^[A-Za-z_][A-Za-z0-9_]*(\.[A-Za-z_][A-Za-z0-9_]*)*$', s):
